...
WORKDIR /go/src/github.com/glarfs/default-backend
COPY main.go .

RUN CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -ldflags '-w -s' -o main github.com/glarfs/default-backend
